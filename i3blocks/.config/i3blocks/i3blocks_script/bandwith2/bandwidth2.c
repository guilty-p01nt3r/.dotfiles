...
void usage(char *argv[]) {
  printf("Usage: %s [-b|B] [-t seconds] [-i interface] [-w Bytes:Bytes] [-c "
         "Bytes:Bytes] [-s] [-h]\n",
         argv[0]);
  printf("\n");
  printf("-b \t\tuse bits/s\n");
  ...
  printf("-w Bytes:Bytes\tSet warning (color orange) for Rx:Tx bandwidth. "
         "(default: none)\n");
  printf("-c Bytes:Bytes\tSet critical (color red) for Rx:Tx bandwidth. "
         "(default: none)\n");
...
void get_values(char **const ifaces, int num_ifaces, time_t *const s,
                ulli *const received, ulli *const sent) {
  FILE *f;
  ...
  *sent = 0;
  while (fgets(line, BUFSIZ - 1, f) != NULL) {
    if (sscanf(line, "%[^:]: %llu %*u %*u %*u %*u %*u %*u %*u %llu", ifname,
               &temp_r, &temp_s) == 3) {
      int i;
...
void display(int const unit, int const divisor, double b, int const warning,
             int const critical) {
  char fmtstr[7];
  ...
  snprintf(fmtstr, sizeof(fmtstr), "%%%d.1lf", divisor > 1000 ? 2 : 1);
...
int main(int argc, char *argv[]) {
  ...
  envvar = getenv("UP_LABEL");
  if (envvar)
    uplabel = envvar;
  envvar = getenv("DOWN_LABEL");
  if (envvar)
    downlabel = envvar;
